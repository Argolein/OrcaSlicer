...
std::string GCode::travel_to(const Point& point, ExtrusionRole role, std::string comment, double z/* = DBL_MAX*/)
{
    ...
    if (this->on_first_layer()) {
        const double initial_layer_travel_acceleration = m_config.get_abs_value("initial_layer_travel_acceleration");
        const double initial_layer_travel_jerk         = m_config.get_abs_value("initial_layer_travel_jerk");
        ...
        if (m_config.default_acceleration.value > 0) {
            if (short_perimeter_travel) {
                if (m_config.travel_short_distance_acceleration.value > 0) {
                    ...
                    if (!used_bridge_acceleration && m_config.outer_wall_acceleration.value > 0) {
