...
std::map<int, DynamicPrintConfig> Sidebar::build_filament_ams_list(MachineObject* obj)
{
    ...
    if (agent && agent->get_printer_agent())
        agent_id = agent->get_printer_agent()->get_agent_info().id;
    ...
    auto build_tray_config = [&agent_id](DevAmsTray const &tray, std::string const &name, std::string ams_id, std::string slot_id) {
        ...
        tray_config.set_key_value("sync_agent_id", new ConfigOptionStrings{agent_id});
